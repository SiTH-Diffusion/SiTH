...
def draw_handpose(canvas, point_2D):
    H, W, C = canvas.shape
    ...
    for i, keyponit in enumerate(point_2D):
        x = keyponit[0]
        y = keyponit[1]
        if x > 0 and y > 0 and x < W and y < H:
            cv2.circle(canvas, (int(x), int(y)), 2, (0, 0, 255), thickness=-1)
    return canvas
...
def vis_meshes(img, kps, alpha=1, radius=3, color=None):
...
def draw_openpose_keypoints(joint_2d, canvas, height=1024, width=1024, skeleton='COCO25'):
    ...
    return: np.array, shape=(height, width, 3), image with keypoints
    '''
